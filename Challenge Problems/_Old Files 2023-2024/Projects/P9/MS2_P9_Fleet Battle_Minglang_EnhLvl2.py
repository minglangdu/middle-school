"""
Name: Minglang Du
Program: Fleet Battle, Enhancement Level 2
Class: 7-C6
"""
import sys
import random
...
def shoot(board, hit, sunk): # shoot at a ship
    new = board[:] # player
    newh = hit[:] # hit list
    news = sunk.copy()
    outcome = ""
    while 1:
        try: # same as place function
            ...
            if (outcome == "~"):
                ...
                newh[y][x] = "H"
                news[outcome] -= 1
            return [outcome, new, newh, news]
        except:
            print("Invalid. Try again.")
            continue

def prompt(shi, hits, opship, sunk, player): # prompt each player to place
    while 1:
...
def autoshoot(board, hit, sunk, level=1):
    moves = []
    nmoves = []
    for y in range(len(hit)):
        for x in range(len(hit[0])):
            if hit[y][x] == "?":
                ...
                if level > 1:
                    f = False
                    if x > 0 and hit[y][x - 1] == 'H':
                        f = True
                    elif x < len(hit[0]) - 1 and hit[y][x + 1] == 'H':
                        f = True
                    elif y > 0 and hit[y - 1][x] == 'H':
                        f = True
                    elif y < len(hit) - 1 and hit[y + 1][x] == 'H':
                        f = True
                    if f:
                        nmoves.append([x, y])
    if nmoves:
...
if input("Players, do you want to play against an ai? (y/n)") == "y":
    ...
    while 1:
        try:
            ai = int(input("Choose a level (1-2)"))
            if ai < 1 or ai > 2:
                raise SyntaxError("Wrong ai level")
            break
        except:
...
while 1: # hitting list
    ...
    s1 = a[3]
    if not ai:
        b = prompt(p2, h2, p1, s2, 2)
    else:
        b = autoshoot(p1, h2, s2, level=ai)
    if not b:
        break
