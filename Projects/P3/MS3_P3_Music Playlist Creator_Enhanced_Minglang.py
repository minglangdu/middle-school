...
def shuffle_playlist():
    ...
    while 1: # stops users from picking an error
        try:
            num = int(input("Choose the playlist number: "))
            key = list(playlists.keys())[num - 1] # the name of the playlist
            random.shuffle(playlists[key])
            break
        except:
            print("Invalid") # stops wrong inputs
...
def search_songs():
    ...
    matches = [] # all of the relevant songs
    for a in song_library.items():
        if (term.lower() in a[0].lower()): # term in song title
            matches.append(a)
            continue # no double matches
        for quality in a[1]: # all attributes
            if term.lower() in str(a[1][quality]).lower():
                matches.append(a)
    print(f"{'Title':27} {'Artist':18} {'Album':20} {'Year':6}",
          f"{'Genre':13} Duration") # same headers as song list
    print("=" * 101) 
    for a in matches:
        song_details = a[1]
        ...
        # Output that song's details using their columns' field widths:
        print(f"{a[0]:27} {artist:18} {album:20}",
              f"{year:6} {genre:13} {minutes:2}:{seconds:02}")
